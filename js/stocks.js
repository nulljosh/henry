        ...
        card.innerHTML = `
            <div class="option-header">
                <div class="option-title">
                    ...
                    <span class="option-type ${option.type}">${option.type.toUpperCase()}</span>
            ...
            <div class="option-details">
                <div class="option-row">
                    ...
                    <span class="option-value">${option.underlying} @ $${option.underlyingPrice.toFixed(2)}</span>
                </div>
                <div class="option-row">
                    <span class="option-label">Expiration:</span>
                    <span class="option-value ${daysClass}">${option.expiration} (${option.daysToExpiration}d)</span>
                </div>
                <div class="option-row">
    ...
    modalBody.innerHTML = `
        <div class="option-card" style="border-left: 4px solid ${option.color}; margin-bottom: 0;">
            ...
            <div class="option-details">
                ...
                <div class="option-row">
                    <span class="option-label">Expiration</span>
                    <span class="option-value ${daysClass}">${option.expiration} (${option.daysToExpiration} days)</span>
                </div>
                <div class="option-row">
                    <span class="option-label">Contracts</span>
                ...
                <div class="option-row">
                    ...
                    <span class="option-value">${option.filledDate} at ${option.filledTime}</span>
                </div>
                ` : ''}
            </div>
        </div>
...
function initializePieChart(pieHoldings, currentTotal) {
    ...
    const activateSlice = (symbol) => {
        ...
        const displayText = item.isOption
            ? `${formatCurrency(item.currentValue, 'USD')}<span>${item.symbol} Option</span>`
            : `${formatCurrency(item.currentValue, item.currency || 'USD')}<span>${item.symbol} • ${formatPercent(item.gainPercent)} target</span>`;
...
function initializeTableView(tableHoldings) {
    ...
    tableHoldings.forEach(item => {
        ...
        const cells = [
            `<a href="${item.url || '#'}" target="_blank" rel="noopener">${item.symbol}</a>`,
            formatShares(item.shares),
            formatPrice(item.price, item.currency),
            formatPercent(item.dailyChangePercent || 0),
